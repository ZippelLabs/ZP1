...
pub fn bytes32_to_m31_limbs(bytes: &[u8; 32]) -> Vec<M31> {
    ...
    for _ in 0..9 {  // We need exactly 9 limbs for 256 bits
        if bit_pos >= 256 {
            limbs.push(M31::ZERO);
            continue;
        }
...
mod tests {
    ...
    fn test_bytes32_to_limbs() {
        ...
        assert_ne!(limbs[0].as_u32(), 0, "Should extract first byte into first limb");
        ...
        assert!(has_nonzero, "Pattern should produce at least one non-zero limb");
    }
}
