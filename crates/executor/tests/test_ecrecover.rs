...
fn test_ecrecover_syscall_valid() {
    ...
    assert_ne!(&output[..], &[0u8; 20], "ECRECOVER should recover a valid address");
    ...
    assert_eq!(&output[..], expected_address, 
        "ECRECOVER address mismatch!\nExpected: {:?}\nGot: {:?}", 
        expected_address, output);
    
    // Verify the trace contains the ECRECOVER delegation
    let trace = cpu.take_trace().unwrap();
    let ecrecover_ops: Vec<_> = trace.rows.iter()
        .filter(|row| matches!(row.mem_op, zp1_executor::trace::MemOp::Ecrecover { .. }))
        .collect();
    
    assert_eq!(ecrecover_ops.len(), 1, "Should have exactly one ECRECOVER operation in trace");
}
...
fn test_ecrecover_invalid_signature() {
    ...
    assert_eq!(&output[..], &[0u8; 20], "Invalid signature should produce zero address");
