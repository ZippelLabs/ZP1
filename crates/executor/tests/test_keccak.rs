...
fn test_keccak256_syscall() {
    ...
    let keccak_ops: Vec<_> = trace.rows.iter()
        .filter(|row| matches!(row.mem_op, zp1_executor::trace::MemOp::Keccak256 { .. }))
        .collect();
    
    assert_eq!(keccak_ops.len(), 1, "Should have exactly one Keccak256 operation in trace");
}
...
fn test_keccak256_empty() {
    ...
    let expected_hash = hex::decode("c5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470")
        .unwrap();
    ...
    // Execute the syscall
    match cpu.step() {
        Err(zp1_executor::ExecutorError::Ecall { syscall_id: 93, .. }) |
        Ok(_) => {
            // Check output
            let mut output = [0u8; 32];
            for i in 0..32 {
