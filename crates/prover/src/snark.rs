...
use crate::recursion::{RecursionError, RecursiveProof};
...
impl StarkVerificationCircuit {
    ...
    pub fn add_merkle_path_constraint(
        &mut self,
        leaf_hash: Fr,
        path: &[Fr],
        path_bits: &[bool],
    ...
    pub fn add_fri_folding_constraint(&mut self, f_x: Fr, f_neg_x: Fr, alpha: Fr, f_folded: Fr) {
    ...
    pub fn add_range_constraint(&mut self, value: Fr, bits: usize) {
        ...
        for _ in 0..bits {
            ...
            let bit = if current.limbs[0] & 1 == 1 {
                Fr::ONE
            } else {
                Fr::ZERO
            };
...
impl Groth16Prover {
    ...
    pub fn setup(&mut self, r1cs: &R1csSystem) -> Groth16VerificationKey {
        ...
        let a_query: Vec<_> = (0..num_vars)
            .map(|i| {
                g1_proj
                    .scalar_mul(&Fr::from_u64(i as u64 + 100))
                    .to_affine()
            })
            .collect();
        let b_g1_query: Vec<_> = (0..num_vars)
            .map(|i| {
                g1_proj
                    .scalar_mul(&Fr::from_u64(i as u64 + 200))
                    .to_affine()
            })
            .collect();
        let b_g2_query: Vec<_> = (0..num_vars).map(|_| g2_gen).collect();
        let h_query: Vec<_> = (0..r1cs.constraints.len())
            .map(|i| {
                g1_proj
                    .scalar_mul(&Fr::from_u64(i as u64 + 300))
                    .to_affine()
            })
            .collect();
        let l_query: Vec<_> = (0..r1cs.num_private)
            .map(|i| {
                g1_proj
                    .scalar_mul(&Fr::from_u64(i as u64 + 400))
                    .to_affine()
            })
            .collect();
...
impl SnarkWrapper {
    ...
    pub fn setup(
        &mut self,
        circuit_params: &CircuitDescription,
    ) -> Result<SnarkVerificationKey, SnarkError> {
        ...
        let vk = SnarkVerificationKey::new(self.config.system, vk_data, circuit_params.clone());
    ...
    pub fn wrap_recursive(
        ...
        let proof_data = match self.config.system {
            SnarkSystem::Groth16 => {
                self.groth16_prove(&recursive_proof.inner_proof, &public_inputs)?
            }
...
impl SnarkVerifier {
    ...
    fn verify_groth16(&self, proof: &SnarkProof) -> Result<bool, SnarkError> {
        if proof.proof_data.len() < 128 {
            return Err(SnarkError::InvalidProofFormat(
                "Groth16 proof too short".into(),
            ));
        }
        ...
        let a = G1Affine::from_bytes(&a_bytes).unwrap_or(G1Affine::generator());
        ...
        let c = G1Affine::from_bytes(&c_bytes).unwrap_or(G1Affine::generator());

        let groth16_proof = Groth16Proof { a, b, c };

        // Convert public inputs to Fr
        let fr_inputs: Vec<Fr> = proof
            .public_inputs
            .iter()
            .map(|m| Fr::from_m31(*m))
            .collect();
...
impl std::fmt::Display for SnarkError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            SnarkError::VerificationKeyMismatch => {
                write!(f, "Verification key does not match proof")
            }
...
mod tests {
    ...
    fn test_snark_error_display() {
        let err = SnarkError::CircuitTooLarge {
            size: 1000,
            max: 500,
        };
        let msg = format!("{}", err);
