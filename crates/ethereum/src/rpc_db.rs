...
use ethers::{
    ...
    types::{Address as EthersAddress, H256, U256 as EthersU256, BlockId, BlockNumber},
};
use revm::{
    db::Database,
    primitives::{
        AccountInfo, Address as RevmAddress, Bytecode, B256, U256 as RevmU256,
        KECCAK_EMPTY,
    },
};
...
impl RpcDb {
    ...
    pub fn new(provider: Arc<Provider<Http>>, runtime: Arc<Runtime>, block_number: Option<u64>) -> Self {
        Self {
            provider,
            runtime,
            block_number,
    ...
    pub fn from_rpc_url(rpc_url: &str, block_number: Option<u64>) -> Result<Self, String> {
        ...
        Ok(Self::new(Arc::new(provider), Arc::new(runtime), block_number))
    }
    
    /// Get block ID for queries.
    fn block_id(&self) -> Option<BlockId> {
        self.block_number.map(|n| BlockId::Number(BlockNumber::Number(n.into())))
    }
    ...
    fn fetch_storage(&self, address: RevmAddress, slot: RevmU256) -> Result<RevmU256, String> {
        ...
        self.runtime.block_on(async {
            ...
            let mut bytes = [0u8; 32];
            value.to_fixed_bytes().iter().enumerate().for_each(|(i, b)| bytes[i] = *b);
